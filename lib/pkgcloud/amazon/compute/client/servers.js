...
exports.createServer = function createServer(options, callback) {
  ...
  securityGroup = this.securityGroup || options['SecurityGroup'] || options.securityGroups;
  if (securityGroup && securityGroup instanceof Array) {
    createOptions['SecurityGroups'] = securityGroup;
  } else if (securityGroup && typeof securityGroup === 'string') {
    createOptions['SecurityGroups'] = [securityGroup];
  }

  securityGroupId = this.securityGroupId || options['SecurityGroupId'] || options.securityGroupIds;
  if (securityGroupId && securityGroupId instanceof Array) {
    createOptions['SecurityGroupIds'] = securityGroupId;
  } else if (securityGroupId && typeof securityGroupId === 'string') {
    createOptions['SecurityGroupIds'] = [securityGroupId];
  }
  ...
  if (options.ipAddress) {
    createOptions.PrivateIpAddress = options.ipAddress;
  }

  if (options.userData) {
    createOptions.UserData = options.userData;
  }

  if (options.blockDeviceMapping) {
    createOptions.BlockDeviceMappings = options.blockDeviceMapping;
  }
