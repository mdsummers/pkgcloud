...
// ####     @vpcId {string} **Optional** id of VPC to associate group with
// #### @callback {function} Continuation to respond to when complete.
//
...
exports.addGroup = function (options, callback) {
  ...
  if (options.vpcId) {
    requestOpts.VpcId = options.vpcId;
  }
...
// ####     @name {string} String name of the group
// ####     @id {string} id of the group (required for non-default VPC)
...
exports.addRules = function (options, callback) {
  if (!options || !options.rules) {
    return errs.handle(
      errs.create({ message: '`rules` is a required option.' }),
      callback
    );
  }

  if (options.name ? options.id : !options.id) {
    return errs.handle(
      errs.create({ message: 'Exactly one of `id` and `name` should be used.' }),
      callback
    );
  }
  ...
  if (options.name) {
    requestOpts.GroupName = options.name;
  } else {
    requestOpts.GroupId = options.id;
  }

  this.ec2.authorizeSecurityGroupIngress(requestOpts, function(err) {
    return err
      ? callback(err)
      : callback(null, true);
  });
